...
**Tools must match the instructions.** `additionalInstructions` and `availableTools` have to agree: if the distilled prompt tells the agent to read files, run tests, or touch a branch, the tools that let it do that must be present. Do **not** default `availableTools` to `[]` and rely only on an explicit source tool list - source frameworks usually leave tool usage implicit in prose. After writing the instructions, re-read them for tool-shaped intent and populate `availableTools` (`filesystem` for file/code work, `git` for repo work - and put a `git.clone` upstream of any code step), recording the inference as a MANIFEST TODO. An agent whose instructions assume filesystem/git access it was never given is the most common broken conversion; see `integration-mapping.md` § "Implicit tool usage". The validator warns on this mismatch.
